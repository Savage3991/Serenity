...
typedef HINSTANCE(WINAPI* LoadLibraryA_t)(const char* lpLibFilename);
...
typedef PVOID(NTAPI* RtlAllocateHeap_t)(std::uint64_t HeapHandle, ULONG Flags, SIZE_T Size);
typedef BOOLEAN(NTAPI* RtlFreeHeap_t)(std::uint64_t HeapHandle, ULONG Flags, PVOID BaseAddress);
typedef NTSTATUS(NTAPI* LdrpHandleTlsData_t)(LDR_DATA_TABLE_ENTRY* pEntry);
typedef DWORD(WINAPI* GetCurrentThreadId_t)();
typedef TEB* (NTAPI* NtCurrentTeb_t)();
typedef void* (WINAPI* AddVectoredExceptionHandler_t)(ULONG First, void* Handler);
...
struct Shared {
	...
	NtCurrentTeb_t NtCurrentTeb;

	AddVectoredExceptionHandler_t AddVectoredExceptionHandler;

	LIST_ENTRY* LdrpTlsList;
	LdrpHandleTlsData_t LdrpHandleTlsData;

	UINT checkpoint;

	std::uintptr_t InternalMapperPage;
	std::size_t InternalMapperSize;
};
